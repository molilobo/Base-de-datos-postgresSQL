...
def connect_postgres():
    ...
    with psycopg.connect(host=config.get(config_type, "Host"), port=config.get(config_type, "Port"),
        ...
        cur = conn.cursor()
        #consult(cur)
        query = ''' CREATE VIEW  Stock_Productos as 
        select i.item_id ,i.description ,stk.quantity from item as i 
        join stock as stk on stk.item_id=i.item_id
        
        
        '''
        #data = cur.execute(query)
       # print(data)
        query2= 'select * from Stock_Productos'
        data2 = cur.execute(query2)
        for row in data2:
           print(row)
    return cur, conn
